...
class CategoryController extends Controller
{
    public function index(Companies $companies, User $user, Category $category)
    {
        ...
        $categorys = Category::orderBy('orderBy', 'ASC')->paginate(8);
    ...
    public function searchCategory(Request $request)
    {
        ...
        $search = Category::with('product')
            ->where('category_title', 'LIKE', '%' . $request->search . '%')
            ->get();
        ...
        return response()->json([
            'data' => $search,
            'status' => 200,
            'success' => 'Erfolgreich eine Suchanfrage gestellt!'
        ]);
    }
